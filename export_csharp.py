...
# Exports complete game logic (layouts, reset, swap) for standalone use in Grasshopper


def export_to_csharp(model=None) -> str:
    """
    Export complete game logic as C# for Grasshopper C# component.
    No dependency on current model state - the script runs the full simulation from parameters.
    Grasshopper inputs: x=W, y=H, z=Steps, (optional) Layout, SwapMode, RoadTopology
    Outputs: A=agent points, B=agent types, C=total utility, D=stats
    """
    # Default config from config.py (standard schema)
    attr_keys = ["T", "P", "R", "W", "S", "H", "G"]
    type_labels = ["Resi", "Firm", "Shop", "Cafe", "Hotel", "Restaurant", "Clinic"]
    pref_attr = {
    ...
    road_names = ["From Layout", "Linear", "Parallel", "Cross", "T-Junction", "Loop"]
    swap_modes = ["pareto", "greedy_total", "greedy_both", "greedy_1"]

    # Build C# config strings
    pa_str = _build_pref_attr_cs(attr_keys, type_labels, pref_attr)
    pg_str = _build_pref_agent_cs(type_labels, pref_agent)
    inf_attr_str = _build_influence_attr_cs(attr_keys, type_labels)  # T=20, others=6
    inf_agent_str = _build_influence_agent_cs(type_labels)

    code = '''// CITY LAB - Full Game Logic for Grasshopper (Rhino)
// Standalone C# script - paste into Grasshopper C# component
// Inputs: x=W, y=H, z=Steps (add params in component; wire number sliders)
// Outputs: A=agents, B=types, C=utility, D=stats, E=attractors
// Add output E: right-click component -> Manage Outputs -> Add Parameter (name: E)
...
public class CityLabGH {
  ...
  public CityLabGH(int w, int h, int reach) {
    W = w; H = h; Reach = reach;
    AttrKeys = new List<string> { ''' + ", ".join(f'"{k}"' for k in attr_keys) + ''' };
    TypeLabels = new List<string> { ''' + ", ".join(f'"{t}"' for t in type_labels) + ''' };
    PrefAttr = ''' + pa_str + ''';
    PrefAgent = ''' + pg_str + ''';
    InfluenceRangeAttr = ''' + inf_attr_str + ''';
    InfluenceRangeAgent = ''' + inf_agent_str + ''';
  ...
  public void ApplyLayout(string typ) {
    ...
    if (CurrentLayout == "Grid") {
      ...
      for (int idx = 0; idx < 5; idx++) {
        int hx = baseHubs[idx,0], hy = baseHubs[idx,1];
        int hxi = Math.Max(1, Math.Min(W-2, hx*W/40));
        int hyi = Math.Max(1, Math.Min(H-2, hy*H/40));
...
int steps = 500;
string layout = "Grid";
string swapMode = "pareto";
string roadTopology = "From Layout";
double scale = 1.0;

if (x != null) { double v; if (double.TryParse(Convert.ToString(x), out v)) w = Math.Max(4, (int)v); }
if (y != null) { double v; if (double.TryParse(Convert.ToString(y), out v)) h = Math.Max(4, (int)v); }
if (z != null) { double v; if (double.TryParse(Convert.ToString(z), out v)) steps = Math.Max(0, (int)v); }
// Add more inputs for Layout, SwapMode, RoadTopology if needed:
// if (layoutInput != null) layout = layoutInput.ToString();
// if (swapInput != null) swapMode = swapInput.ToString();
// if (roadInput != null) roadTopology = roadInput.ToString();

var model = new CityLabGH(w, h, 6);
model.SwapMode = swapMode;
model.RoadTopology = roadTopology;
...
E = model.GetAttractorPoints(scale);
'''
    return code


def _build_pref_attr_cs(attr_keys, type_labels, pref_attr):
    lines = []
    for t in type_labels:
        inner = ", ".join(f'{{"{k}", {pref_attr.get(t, {}).get(k, 0.0)}}}' for k in attr_keys)
        lines.append(f'{{"{t}", new Dictionary<string, double> {{ {inner} }}}}')
    return "new Dictionary<string, Dictionary<string, double>> {\n  " + ",\n  ".join(lines) + "\n}"


def _build_pref_agent_cs(type_labels, pref_agent):
    lines = []
    for t in type_labels:
        inner = ", ".join(f'{{"{tt}", {pref_agent.get(t, {}).get(tt, 0.0)}}}' for tt in type_labels)
        lines.append(f'{{"{t}", new Dictionary<string, double> {{ {inner} }}}}')
    return "new Dictionary<string, Dictionary<string, double>> {\n  " + ",\n  ".join(lines) + "\n}"


def _build_influence_attr_cs(attr_keys, type_labels):
    """T=20, others=6"""
    lines = []
    for k in attr_keys:
        r = 20 if k == "T" else 6
        inner = ", ".join(f'{{"{t}", {r}}}' for t in type_labels)
        lines.append(f'{{"{k}", new Dictionary<string, int> {{ {inner} }}}}')
    return "new Dictionary<string, Dictionary<string, int>> {\n  " + ",\n  ".join(lines) + "\n}"


def _build_influence_agent_cs(type_labels):
    """All 6"""
    lines = []
    for s in type_labels:
        inner = ", ".join(f'{{"{t}", 6}}' for t in type_labels)
        lines.append(f'{{"{s}", new Dictionary<string, int> {{ {inner} }}}}')
    return "new Dictionary<string, Dictionary<string, int>> {\n  " + ",\n  ".join(lines) + "\n}"
